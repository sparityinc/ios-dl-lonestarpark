...
- (void)viewDidLoad
{
    ...
    self.adwLbl.text = [NSString stringWithFormat:@"%@ Account:",CLIENT_NAME];
    self.adwDisLbl.text =[NSString stringWithFormat:@"%@ Account is a permanent account that allows a user to login via mobile.",CLIENT_NAME];
    [self.accountBtn setTitle:[NSString stringWithFormat:@"%@ Account",CLIENT_NAME] forState:UIControlStateNormal];
    
    self.accountBtn.titleLabel.textColor = [UIColor whiteColor];
    [self prepareView];
    if (!IS_IPHONE5) {
...
//                UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"Secure Registration" message: [NSString stringWithFormat:@"Do you already have an account with %@ account?",CLIENT_NAME] delegate:self cancelButtonTitle:@"NO" otherButtonTitles:@"YES", nil];
//                [alertView show];
