...
//            self.AddFundsBtn.hidden = YES;
            ...
            [self.withDrawalBtn setFrame:CGRectMake(self.withDrawalBtn.frame.origin.x+50, self.withDrawalBtn.frame.origin.y-10, 150, self.withDrawalBtn.frame.size.height)];
            [self.currentBalanceView setFrame:CGRectMake(self.currentBalanceView.frame.origin.x, self.currentBalanceView.frame.origin.y, self.currentBalanceView.frame.size.width, self.currentBalanceView.frame.size.height-10)];
            [self.AddFundsBtn setFrame:CGRectMake(self.AddFundsBtn.frame.origin.x, self.AddFundsBtn.frame.origin.y-10, self.AddFundsBtn.frame.size.width+48, self.AddFundsBtn.frame.size.height)];
        }
    ...
    [apiWrapper getCurrentBalanceForAccount:nil success:^(NSDictionary *_userInfo)
     {
         ...
         if ([succesMes isEqualToString:@"success"]){
             ...
             UIAlertView *alert=[[UIAlertView alloc]initWithTitle:CLIENT_NAME message:@"The server could not process your request at this time, please try later" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
             [alert show];
         }
...
{
    ZLAPIWrapper *apiWrapper = [ZLAppDelegate getApiWrapper];
    [[LeveyHUD sharedHUD] appearWithText:@"Retrieving Payment Information..."];
