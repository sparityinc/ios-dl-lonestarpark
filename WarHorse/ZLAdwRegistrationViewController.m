...
//therman adding playerid to ODP
@property (strong, nonatomic) IBOutlet UITextField *mywinnersIdTFODP;
@property (strong, nonatomic) IBOutlet UIImageView *mywinnersIdImgViewODP;
@property (strong, nonatomic) IBOutlet UITextField *confirmMyWinnersIdTfODP;
@property (strong, nonatomic) IBOutlet UIImageView *confirmMyWinnersIdImgODP;
...
@property (strong, nonatomic) UITapGestureRecognizer *singleTapGestureRecognizer, *singleTapGestureRecognizer1,*singleTapGestureRecognizer2,*infoTapRecognizer,*singleTapGestureRecognizer3;
...
@property (strong, nonatomic) IBOutlet UIScrollView *odpScrollView;
@property (nonatomic,assign)BOOL isStateOfResidenceInfoEnabled;
...
- (void)viewDidLoad
{
    ...
    self.mywinnersIdTF.placeholder = [NSString stringWithFormat:@"%@ ID",CLIENT_NAME];
    self.confirmMyWinnersIdTf.placeholder =[NSString stringWithFormat:@"Confirm %@ ID",CLIENT_NAME];

    UIView *paddingView1 = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 6, 40)];
    self.mywinnersIdTFODP.leftView = paddingView1;
    self.mywinnersIdTFODP.leftViewMode = UITextFieldViewModeAlways;
    self.mywinnersIdTFODP.rightView = paddingView1;
    self.mywinnersIdTFODP.rightViewMode = UITextFieldViewModeAlways;
    
    UIView *paddingView2 = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 6, 40)];
    self.confirmMyWinnersIdTfODP.leftView = paddingView2;
    self.confirmMyWinnersIdTfODP.leftViewMode = UITextFieldViewModeAlways;
    self.confirmMyWinnersIdTfODP.rightView = paddingView2;
    self.confirmMyWinnersIdTfODP.rightViewMode = UITextFieldViewModeAlways;


    NSString *playerIdLabel =[self.registerAppConfigDict valueForKey:@"player_id_label"];
    
    self.mywinnersIdTFODP.placeholder = [NSString stringWithFormat:@"%@",playerIdLabel];
    self.confirmMyWinnersIdTfODP.placeholder =[NSString stringWithFormat:@"Confirm %@",playerIdLabel];
    ...
    self.singleTapGestureRecognizer3 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(resignKeyBoard)];
    self.singleTapGestureRecognizer3.numberOfTapsRequired = 1;
    [self.odpScrollView addGestureRecognizer:self.singleTapGestureRecognizer3];
    ...
    [[WarHorseSingleton sharedInstance] setStateCodeString:[self.registerAppConfigDict valueForKey:@"reg_default_state"]];
    
    self.isStateOfResidenceInfoEnabled = [[self.registerAppConfigDict valueForKey:@"state_of_residence_info_enabled"]boolValue];
    if (self.isStateOfResidenceInfoEnabled == YES) {
        NSString *StateOfResidence = [self.registerAppConfigDict valueForKey:@"state_of_residence_info"];
        lblInfo = [[UILabel alloc] initWithFrame:CGRectMake(10.0, self.nextBtn.frame.origin.y + 43.0, 300.0,40.0)];
        lblInfo.numberOfLines = 0;
        lblInfo.backgroundColor = [UIColor colorWithRed:250.0/255.0 green:241.0/255.0 blue:202.0/255.0 alpha:1];
        lblInfo.text = StateOfResidence;

      //  lblInfo.text = [NSString stringWithFormat:@"Only %@ residents are permitted to open accounts \n for online wagering.",[self.registerAppConfigDict valueForKey:@"reg_default_state"]];
...
- (void)prepareView
{
    if ([[[WarHorseSingleton sharedInstance] userType] isEqualToString:@"ADW"]){
        ...
        self.headerNameLabel.text = [NSString stringWithFormat:@"Create %@ Account",CLIENT_NAME];
        [self resignKeyBoard];
        
        [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%@",CLIENT_ID] forKey:@"ClientId"];
        ...
        //THERMAN add in to check the player id flags
...
//            [self.odpScrollView setContentSize:CGSizeMake(self.view.bounds.size.width, self.view.bounds.size.height-64)];
//            //make changes in height if necessary
//            self.odpScrollView.frame = CGRectMake(0, 64, self.view.bounds.size.width, self.view.bounds.size.height-64);
        }
        else //iphone 4/4s
        {
            ...
            if([[UIDevice currentDevice].systemVersion hasPrefix:@"7"]) //iOS 7.0 >
            {
                ...
                self.odpScrollView.frame = CGRectMake(0, 0, self.view.bounds.size.width, self.view.bounds.size.height-64);
            }
            else //iOS 6.1 <
            {
        ...
        if (!self.mywinnerIDEnable){
            ...
            NSString *infoStr =  [self.registerAppConfigDict valueForKey:@"newreg_s1_info"];
            if (infoStr.length == 0){
                self.mywinnersIdTFODP.hidden = YES;
                self.mywinnersIdImgViewODP.hidden = YES;
                self.confirmMyWinnersIdTfODP.hidden = YES;
                self.confirmMyWinnersIdImgODP.hidden = YES;
                
                self.registerBtn.frame = CGRectMake(31, 302, self.registerBtn.frame.size.width, self.registerBtn.frame.size.height);
                
            }else{
                self.mywinnersIdTFODP.hidden = NO;
                self.mywinnersIdImgViewODP.hidden = NO;
                self.confirmMyWinnersIdTfODP.hidden = NO;
                self.confirmMyWinnersIdImgODP.hidden = NO;
                
            }
            
            
        }
        else if (self.mywinnerIDEnable)
        {
            self.mywinnersIdTFODP.hidden = NO;
            self.mywinnersIdImgViewODP.hidden = NO;
            self.confirmMyWinnersIdTfODP.hidden = NO;
        ...
        [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%@-ODP",CLIENT_ID] forKey:@"ClientId"];
        [[NSUserDefaults standardUserDefaults] synchronize];
        ...
        [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%@-DV",CLIENT_ID] forKey:@"ClientId"];
        [[NSUserDefaults standardUserDefaults] synchronize];
        [self.ondayPassView removeFromSuperview];
...
- (IBAction)nextToPersonalInformation:(id)sender
{
    ...
    if (![[WarHorseSingleton sharedInstance] isInternetConnectionAvailable]) {
        ...
        [self alertViewMethod:CLIENT_NAME messageText:@"Unable to connect to the server, please check your internet connection"];
        return;
    }
    ...
    if (![[[WarHorseSingleton sharedInstance] stateString] isEqualToString:[self.registerAppConfigDict valueForKey:@"reg_default_state"]])
    {
        [self alertViewMethod:@"Information" messageText:[NSString stringWithFormat:@"Only %@ residents are permitted to open accounts \n for online wagering.",[self.registerAppConfigDict valueForKey:@"reg_default_state"]]];
    ...
    if (self.mywinnerIDEnable){
        if (![self.mywinnersIdTF.text isEqualToString:self.confirmMyWinnersIdTf.text]) {
            ...
            [self alertViewMethod:@"Information" messageText:[NSString stringWithFormat:@"%@ID doesn't match",CLIENT_NAME]];
            return;
        }
    }
    [self resignKeyBoard];
...
- (IBAction)createAccount:(id)sender
{
    ...
    if ([self.usernameTF.text length] < 5){
        [self alertViewMethod:@"Invalid Username" messageText:@"Your username must contain at least 6 and a maximum of 12 characters"];
        return;
...
- (void)resignKeyBoard
{
    ...
    //therman adding resign to player id tf
    [self.mywinnersIdTFODP resignFirstResponder];
    [self.confirmMyWinnersIdTfODP resignFirstResponder];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if ([[tempDict allKeys]  containsObject:@"Error"]){
        ...
        [self alertViewMethod:CLIENT_NAME messageText:cityStr];
        return;
	...
	NSLog(@"zipcodestate%@",zipCodeState);
//        if (![zipCodeState isEqualToString:[self.registerAppConfigDict valueForKey:@"reg_default_state"]]){
//            [self alertViewMethod:@"Information" messageText:[NSString stringWithFormat:@"Please enter valid Zip Code of %@",[self.registerAppConfigDict valueForKey:@"reg_default_state"]]];
//            return;
//           
//        }
...
- (IBAction)registerBtnClicked:(id)sender
{
    ...
    if ([self.registerType isEqualToString:@"ODP"]){
        ...
        //Therman adding player ID validation to seeif the fields match
        if (![self.mywinnersIdTFODP.text isEqualToString:self.confirmMyWinnersIdTfODP.text]) {
            [self alertViewMethod:@"Information" messageText:@"Player ID's don't match"];
            return;
        }
            ...
            return;
                ...
                ondayPassDict = @{@"consumerType":@"ODP",
                         ...
                         // @"playerID":self.rewardsIdTxtFld.text,
                          //Therman adding player id validation
                          @"playerID":self.mywinnersIdTFODP.text};
    }else{
    ...
    [apiWrapper registerForAnonymousUsers:ondayPassDict success:^(NSDictionary *userDict){
       ...
       // [spinner stopAnimating];
        ...
        if ([self.registerType isEqualToString:@"ODP"]){
            ...
            //therman initializing player id textfields empty
            self.mywinnersIdTFODP.text = @"";
            self.confirmMyWinnersIdTf.text = @"";
