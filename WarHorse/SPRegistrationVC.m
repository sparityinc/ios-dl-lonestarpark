...
@property (strong, nonatomic) IBOutlet UIImageView *emailImgView;
@property (strong, nonatomic) IBOutlet UITextView *termsAndConditionTextView;
@property (strong, nonatomic) IBOutlet UILabel *headerLabel;
@property (nonatomic,assign) BOOL isIdeologyEnabled;
@property (strong, nonatomic) IBOutlet UIImageView *phoneImgView;
...
- (void)viewDidLoad
{
    ...
    self.isIdeologyEnabled = [[self.registerAppConfigDict valueForKey:@"yes_ideology_enabled"]boolValue];
    NSLog(@"%d",self.isIdeologyEnabled);
    if (self.isIdeologyEnabled == NO) {
        [self.ssnTxtFld setHidden:YES];
        [self.ssnImgView setHidden:YES];
        
        [self.emailTxtFld setFrame:CGRectMake(self.emailTxtFld.frame.origin.x, self.emailTxtFld.frame.origin.y-46, self.emailTxtFld.frame.size.width, self.emailTxtFld.frame.size.height)];
        [self.emailImgView setFrame:CGRectMake(self.emailImgView.frame.origin.x, self.emailImgView.frame.origin.y-46, self.emailImgView.frame.size.width, self.emailImgView.frame.size.height)];
        
        [self.phoneTxtFld setFrame:CGRectMake(self.phoneTxtFld.frame.origin.x, self.phoneTxtFld.frame.origin.y-46, self.phoneTxtFld.frame.size.width, self.phoneTxtFld.frame.size.height)];
        
        [self.phoneImgView setFrame:CGRectMake(self.phoneImgView.frame.origin.x, self.phoneImgView.frame.origin.y-46, self.phoneImgView.frame.size.width, self.phoneImgView.frame.size.height)];

        
        [self.promotionalCodeTxtFld setFrame:CGRectMake(self.promotionalCodeTxtFld.frame.origin.x, self.promotionalCodeTxtFld.frame.origin.y-46, self.promotionalCodeTxtFld.frame.size.width, self.promotionalCodeTxtFld.frame.size.height)];
        [self.promotionalImgView setFrame:CGRectMake(self.promotionalImgView.frame.origin.x, self.promotionalImgView.frame.origin.y-46, self.promotionalImgView.frame.size.width, self.promotionalImgView.frame.size.height)];
        
       [self.nextBtn setFrame:CGRectMake(self.nextBtn.frame.origin.x, self.nextBtn.frame.origin.y-46, self.nextBtn.frame.size.width, self.nextBtn.frame.size.height)];
        [self.textView1 setFrame:CGRectMake(self.textView1.frame.origin.x, self.textView1.frame.origin.y-46, self.textView1.frame.size.width, self.textView1.frame.size.height)];
        
    }
    else{
        [self.ssnTxtFld setHidden:NO];
        [self.ssnImgView setHidden:NO];
    }
    ...
    if (self.isStateOfResidenceInfoEnabled == YES) {
        ...
        [self.textView1 setHidden:YES];
    }

    self.headerLabel.text = [NSString stringWithFormat:@"Create %@ Account",CLIENT_NAME];
    
    
   // self.textView1.text = [NSString stringWithFormat:@"**Please note that only %@ residents are permitted to open wager online at %@.com",[self.registerAppConfigDict valueForKey:@"reg_default_state"],CLIENT_NAME];
    ...
    NSLog(@"%@",self.statesCodesArray);
    
   // NSDictionary *localDic = self.statesCodesArray;
    
    NSString *stateCode = [self.statesCodesArray valueForKey:@"StateCode"];
    self.pickerSelectedStateCodeString = stateCode;
    
    [[WarHorseSingleton sharedInstance] setStateCodeString:@"TX"];
...
{
    [self resignKeyBoard];
    if ([self.accountNumberTxtFld.text isEqualToString:@""] || [self.accountPinTxtFld.text isEqualToString:@""] || [self.emailTxtFld.text isEqualToString:@""] || [self.phoneTxtFld.text isEqualToString:@""]) {
        [self alertViewMethod:@"Information" messageText:@"Please fill all the fields"];
        
        return;
    }
    if (self.isIdeologyEnabled == YES) {
        
    
    if ([self.ssnTxtFld.text isEqualToString:@""]) {
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    if (textField == self.pswdTxtFld) {
        [self.confirmPswdTxtFld becomeFirstResponder];
    }
