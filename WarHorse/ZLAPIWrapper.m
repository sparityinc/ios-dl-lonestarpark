...
- (id)init
{
    ...
    if (self) {
        ...
        self.baseURL = [NSURL URLWithString:kAPIBaseUrlEndPoint];
        ...
        RKResponseDescriptor *myBetsResponseDescriptor3 = [RKResponseDescriptor responseDescriptorWithMapping:myBetsMapping pathPattern:mybetsfinal keyPath:@"response-content" statusCodes:RKStatusCodeIndexSetForClass(RKStatusCodeClassSuccessful)];
        ...
        RKResponseDescriptor *responseDescriptor7 = [RKResponseDescriptor responseDescriptorWithMapping:preLoginResponseContentMapping pathPattern:[NSString stringWithFormat:@"%@/services/loadData",BASE_CONTEXT_NAME_CMS]keyPath:@"response-content" statusCodes:RKStatusCodeIndexSetForClass(RKStatusCodeClassSuccessful)];
        
        [objectManager addResponseDescriptor:responseDescriptor7];
        
        [[RKObjectManager sharedManager].HTTPClient setDefaultHeader:@"client_id" value:[NSString stringWithFormat:@"%@",CLIENT_ID]];
                                                   ...
                                                   success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                                       ...
                                                       if( [responseObject isKindOfClass:[NSDictionary class]]){ //[operation.response statusCode] == 201 &&
                                                           ...
                                                           if( [_status isEqualToString:@"success"]){
                                                               ...
                                                               if (failure) {
                                                                   dispatch_async(dispatch_get_main_queue(), ^{
                                                                       NSError * error = [NSError errorWithDomain:[NSString stringWithFormat:@"%@",CLIENT_ID] code:400 userInfo:_dict];
                                                                       failure(error);
                                                                   });
                                                               }
                                                           }
...
- (void)preloginBanners:(NSDictionary *)params success:(void (^)(NSDictionary* _userInfo))success failure:(void (^)(NSError *error))failure
{
    ...
    [[RKObjectManager sharedManager] setRequestSerializationMIMEType:RKMIMETypeJSON];
    [[[RKObjectManager sharedManager] HTTPClient] setParameterEncoding:AFJSONParameterEncoding];
    [[[RKObjectManager sharedManager] HTTPClient] postPath:[NSString stringWithFormat:@"%@/services/loadData",BASE_CONTEXT_NAME_CMS] parameters:params
                                                   success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                                       if([operation.response statusCode] == 201 && [responseObject isKindOfClass:[NSDictionary class]]){
                                                           //NSString *succesMes = [responseObject valueForKey:@"response-status"];
                                                           NSDictionary *userInfo = responseObject;
...
- (void)userDigitalVoucher:(NSDictionary *)dvDeatails success:(void (^)(NSDictionary* _userInfo))success failure:(void (^)(NSError *error))failure{
    ...
    [[RKObjectManager sharedManager].HTTPClient setDefaultHeader:@"client_id" value:[NSString stringWithFormat:@"%@-ODP",CLIENT_ID]];
    [[RKObjectManager sharedManager].HTTPClient setDefaultHeader:@"user_pin" value:@""];
    [[RKObjectManager sharedManager].HTTPClient setDefaultHeader:@"account_id" value:@""];
    [[RKObjectManager sharedManager].HTTPClient setDefaultHeader:@"user_id" value:@""];
    [[RKObjectManager sharedManager].HTTPClient setDefaultHeader:@"user_password" value:@""];
