...
#import "LeveyHUD.h"
#import "SPConstant.h"
//therman adding terms and conditions
...
//therman terms and conditions checkbox
@property (strong, nonatomic) IBOutlet UIButton *checkBoxBtn_ADW;
@property (strong, nonatomic) IBOutlet UIButton *checkBoxBtn_ODP;
@property (strong, nonatomic) IBOutlet UIButton *checkBoxBtn_DV;
//therman terms and conditions checkbox
@property (nonatomic,retain) IBOutlet UIButton *termsAndConditionsButton_ADW;
@property (nonatomic,retain) IBOutlet UIButton *termsAndConditionsButton_ODP;
@property (nonatomic,retain) IBOutlet UIButton *termsAndConditionsButton_DV;
...
//therman adding terms and conditions checkbox
...
- (void)viewDidLoad
{
    ...
    [self.segmentedView setTitle:CLIENT_NAME forSegmentAtIndex:0];
    ...
    UIView *userNamePadding = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
    ...
    UIView *passWordPadding = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
...
- (IBAction)segmentedBtn:(id)sender
{
    ...
    if ([[self.segmentedView titleForSegmentAtIndex:self.segmentedView.selectedSegmentIndex] isEqualToString:CLIENT_NAME]){
...
- (IBAction)login_Clicked:(id)sender
{
    ...
    UIAlertView *alert;
    if ([typeOfLoginStr isEqualToString:@"ADW"] && self.checkBoxBtn_ADW.selected == NO)
    {
        [self alertViewMethod:@"Information" messageText:@"Please agree Terms and Conditions"];
        return;
    }

    else if ([typeOfLoginStr isEqualToString:@"ODP"] && self.checkBoxBtn_ODP.selected == NO){
    ...
    if (![[WarHorseSingleton sharedInstance] isInternetConnectionAvailable]) {
        UIAlertView *alert=[[UIAlertView alloc]initWithTitle:CLIENT_NAME message:@"Unable to connect to the server, please check your internet connection" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
        return;
    }
    ...
    [apiWrapper authenticateUser:userNameandPin withPassword:userPasswordandPin Pin:nil Lat:[[NSUserDefaults standardUserDefaults] valueForKey:@"Latitude"] andLon:[[NSUserDefaults standardUserDefaults] valueForKey:@"Longitude"]
                         success:^(NSDictionary* _userInfo){
                             ...
                             if ([[_userInfo valueForKey:@"response-status"] isEqualToString:@"success"]){
                                 ...
                                 //THERMAN
                                 if ([typeOfLoginStr isEqualToString:@"ADW"] && self.checkBoxBtn_ADW.selected == YES)
                                 {
                                     ...
                                     //THERMAN
                                     if ([userStatusStr isEqualToString:@"ACCOUNT"] && self.checkBoxBtn_ODP.selected == YES){
                                     ...
                                     //THERMAN
                                     if ([userStatusStr isEqualToString:@"ACCOUNT"] && self.checkBoxBtn_DV.selected == YES){
                                         [[NSUserDefaults standardUserDefaults] setValue:[[_userInfo valueForKey:@"response-content"] valueForKey:@"accountId"] forKey:@"DVAccountId"];
                                     ...
                                     //THERMAN
                                     else if ([userStatusStr isEqualToString:@"ACCOUNT"] && self.checkBoxBtn_DV.selected == NO){
